...
private:
    struct ShopSlot
    {
        ...
        float minY       = 0.0f;
        float maxY       = 0.0f;
        TextObject* priceLabel = nullptr;   // kept for SOLD update
    };
    ...
    TextObject* coinDisplay = nullptr;   // live coin counter shown on the page
    Player*     playerRef   = nullptr;   // set on Open(), used for coin display refresh
    ...
    void BuildUI(std::vector<DrawableObject*>& objectsList)
    {
        ...
        auto pushImage = [&](DrawableObject* obj)
        {
            if (obj)
            {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            // Clone card layers
            pushImage(cloneImage(card->GetBackground(),  cardPos, cardWidth, -cardHeight));
            pushImage(cloneImage(card->GetStarOverlay(), cardPos, cardWidth, -cardHeight));
            pushImage(cloneImage(card->GetTypeIcon(),    cardPos, cardWidth, -cardHeight));
            pushImage(cloneImage(card->GetVisual(),      cardPos, cardWidth, -cardHeight));
            pushImage(cloneImage(card->GetCardFrame(),   cardPos, cardWidth, -cardHeight));
            ...
            // Price label below card
            {
                ...
                priceText->SetPosition(glm::vec3(
                    cardPos.x,
                    cardPos.y - cardHeight * 0.5f - 30.0f,
                    812.0f));
    ...
    void ClearUI(std::vector<DrawableObject*>& objectsList)
    {
        ...
        // Nullify price label pointers (the objects were just deleted above)
...
public:
    ...
    // -------------------------------------------------------
    bool LoadPoolData(const std::string& patternFile,
                      const std::string& cardFile,
                      const std::string& cardDescFile,
    ...
    // -------------------------------------------------------
    // Returns true if the click was consumed (hit the shop UI)
    bool HandleMouseClick(const glm::vec3& mousePos,
                          ...
                          std::vector<DrawableObject*>& objectsList)
    {
        ...
        for (ShopSlot& slot : shopSlots)
        {
                ...
                mousePos.y >= slot.minY && mousePos.y <= slot.maxY)
            {
                ...
                {
                    std::cout << "[Shop] Not enough coins! Need "
                              << slot.price << ", have " << player.GetCoins() << "\n";
                    return true;   // consumed — just can't afford it
                }

                // Purchase
                player.SpendCoins(slot.price);
                std::cout << "[Shop] Bought '" << slot.card->getName()
                          << "' for " << slot.price
                          << " coins. Remaining: " << player.GetCoins() << "\n";
